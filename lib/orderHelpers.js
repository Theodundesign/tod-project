...
export const updatePaymentStatus = (currentOrder, newPaymentStatus, paymentReference = '') => {
  ...
  if (newPaymentStatus === PAYMENT_STATUSES.PAID) {
    const newStatus = currentOrder.status === ORDER_STATUSES.COMPLETED ? ORDER_STATUSES.COMPLETED : ORDER_STATUSES.PAID
    ...
    if (!lastEntry || lastEntry.status !== ORDER_STATUSES.PAID) {
...
export const getStatusColor = (status) => {
  const colorMap = {
    ...
    [ORDER_STATUSES.PAID]: '#34d399', // emerald
